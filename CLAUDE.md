...
A personal GitHub Pages blog. Content is written in markdown and published via GitHub Pages. Claude Code skills in `.claude/skills/` provide reusable workflows for investigation, planning, auditing, documentation, and architecture visualization. They chain together (e.g., `investigate → rectify → implement-worktree`) and use subagents for parallel exploration.
...
Run tests relevant to the project's static site generator or build tooling as they are set up. If no test suite exists, verify changes by building the site locally and checking output.

## **5. Pre-commit Hooks**

If hooks are configured, install them after cloning: `pre-commit install`
